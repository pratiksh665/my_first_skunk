...
public class SkunkController
{
	...
	public void rollScore(Player activePlayer, int chipsPenalty)
	{
		...
		//Added business logic for chip penalty
		if(chipsPenalty == 4)
		{
	...
	public boolean run()
	{
		...
		for (int playerNumber = 0; playerNumber < numberOfPlayers; playerNumber++)
		{
			...
			//Updated Player constructor to take String args instead of Integer args
			this.players.add(new Player(playerName));
		}
		activePlayerIndex= 0; 	// Initialized the declared instance variable called activePlayerIndex
		activePlayer=players.get(activePlayerIndex); // updating the instance variable
		...
		while (gameNotOver)
		{
			...
			while (wantsToRoll)
			{
				...
				{
					ui.println("you got double skunk; pay 4 chips"); // added user message
					rollScore(activePlayer,4); //Added method call
					wantsToRoll = false;
					break;
				}
				else if (skunkDice.getLastRoll() == 3)
				{
					...
					kitty += 2;
					rollScore(activePlayer,2);  //Added method call
					wantsToRoll = false;
					break;
				}
				else if (skunkDice.getDie1().getLastRoll() == 1 || skunkDice.getDie2().getLastRoll() == 1)
				{
					...
					rollScore(activePlayer,1);  //Added method call
					wantsToRoll = false;
					break;
				}
		...
		for (int i = activePlayerIndex, count = 0; count < numberOfPlayers - 1; i = (i++) % numberOfPlayers, count++)
		{
			...
			while (wantsToRoll)
			{
				...
				if (skunkDice.getLastRoll() ==2) // updated to match the code below
				{
					rollScore(activePlayer,4); //Added method call
					wantsToRoll = false;
					break;
